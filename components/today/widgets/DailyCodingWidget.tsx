...
/**
 * DailyCodingWidget
 * Dashboard widget for displaying daily competitive programming challenges
 * from LeetCode and GeeksforGeeks, with difficulty badges, tags, and problem links.
 */
...
export const DailyCodingWidget: React.FC<DailyCodingWidgetProps> = ({
  ...
  return (
    <a
      ...
      className={`group block ${isCompactHeight ? 'p-2.5' : 'p-3.5'} bg-[var(--color-bg-surface)] hover:bg-[var(--color-bg-subtle)] border border-[var(--color-border)] ${accentBorder} rounded-[var(--card-radius)] shadow-[var(--card-shadow)] hover:shadow-[var(--card-hover-shadow)] transition-all duration-200 cursor-pointer select-none`}
    >
      {/* Header Row: Platform Tag + Difficulty + Quick Open Arrow */}
      <div className={`flex items-center justify-between gap-2 ${isCompactHeight ? 'mb-1.5' : 'mb-2'}`}>
